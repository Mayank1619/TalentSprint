...
test("administrator can inspect authoring controls and question library actions", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Users" })).toHaveClass(/selected/);
  ...
  await page.getByRole("button", { name: "Edit Invited Examiner" }).click();
  await page.getByLabel("Examiner name").last().fill("Updated Examiner");
  await page.getByLabel("Examiner email").last().fill("updated.examiner@example.com");
  await page.getByRole("button", { name: "Save examiner" }).click();
  await expect(page.getByText("Examiner details updated.")).toBeVisible();
  await expect(page.getByText("updated.examiner@example.com")).toBeVisible();
  await page.getByRole("button", { name: "Disable Updated Examiner" }).click();
  await expect(page.getByText("Examiner access disabled.")).toBeVisible();
  await page.getByRole("button", { name: "Enable Updated Examiner" }).click();
  await expect(page.getByText("Examiner access enabled.")).toBeVisible();
  await page.getByRole("button", { name: "Remove Updated Examiner" }).click();
  await expect(page.getByText("Examiner access removed.")).toBeVisible();
  await expect(page.getByText("updated.examiner@example.com")).toHaveCount(0);
